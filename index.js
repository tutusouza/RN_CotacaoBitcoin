...
for(let l=1; l<linhas.length; l++) {
	let celulas1 = [];
	if(qtdLinhas==(linhas.length-2)) celulas1 = ['31.12.2019', 3709.4, 3815.1, 3819.6, 3658.8];
...
const learningRate = 0.000000001; //o valor da learningRate deve ser inversamente proporcional aos valores calculado na RN,
//normalmente se utiliza dois zeros depois da virgula no learningRate para cada casa decimal nos valores calculados na RN
//porem nao é uma verdade universal, entao essa esse valor deve ser testado até que se arrume um valor procimo do esperado
...
const arrInput = [[6984.8, 7190.0, 7518.9, 6802.6]]; // 13.05.2019
...
model.fit(x, y, {epochs: 500}).then(() => {
	...
	console.log(`Fechamento: R$ ${Number(output[0]).toFixed(1)}`);
	console.log(`Abertura:   R$ ${Number(output[1]).toFixed(1)}`);
	console.log(`Máxima:     R$ ${Number(output[2]).toFixed(1)}`);
	console.log(`Mínima:     R$ ${Number(output[3]).toFixed(1)}`);
});
